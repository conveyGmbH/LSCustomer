...
(function (global, factory) {
    ...
    function floating(o) {
        ...
        var animationName = animationNameSuffix + instance
        ...
        var keyframes = ' @keyframes '+ animationName + ' { '+
            Array.apply(null, {
                ...
                return a.percent + '% { transform: translate(' + 
                    a.width + 'px, ' + 
                    a.height + 'px); opacity: ' +
                    a.opacity + '; font-size: '+
                    a.size + 'px; }\n';
        ...
        function createFloater(cssText) {
            var floater = document.createElement('div');
            floater.innerHTML = o.content;
            floater.style.cssText = cssText;
            floater.addEventListener('animationend', function(event) {
                if (event.animationName.substr() === animationName) {
        ...
        for (var i = 0; i < o.number; i++) {
            createFloater('position: absolute; left: '+ o.left +'; top: '+ o.top +'; z-index: 1000; font-family: color-emoji; font-size: ' +
                size +
                'px; transform: translateY(-'+ o.height +'); animation: ' + animationName + ' ' +
                o.duration +
                's linear ' +
                i * Math.random() +
                's ' +
