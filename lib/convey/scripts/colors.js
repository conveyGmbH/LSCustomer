    ...
    WinJS.Namespace.define("Colors", {
            ...
            if (!Colors.corsAwareCssRuleAccess) {
                ...
                if (server) {
                    ...
                    if (!Colors._baseHref) {
                        ...
                        for (var i = 0; i < scripts.length; i++) {
                            if (scripts[i] && scripts[i].src) {
                                pos = scripts[i].src.indexOf("lib/convey/scripts/colors.js");
                                if (pos > 0) {
                                    Colors._baseHref = scripts[i].src.substr(0, pos);
                                    break;
                                }
                            }
                        }
                ...
                ret = WinJS.xhr({ url: url }).then(function xhrSuccess(res) {
                    ...
                    Log.print(Log.l.error, "Error loading CSS file " + filename + " Error:" + JSON.Stringify(err));
                    return WinJS.Promise.as();
                });
            }
            Log.ret(Log.l.trace);
        ...
        ColorsClass: WinJS.Class.define(
            ...
            function ColorsClass(colorSettings, forceColors) {
                ...
                var loadCssPromise = Colors.loadCSSfile("css/syscolors.css");
                ...
                if (Colors._prevIsDarkTheme !== null && Colors._prevIsDarkTheme === Colors.isDarkTheme) {
                    ...
                    var loadCssPromiseList = [];
                    loadCssPromiseList.push(Colors.loadCSSfile(Colors.isDarkTheme ? "lib/WinJS/css/ui-dark.min.css" : "lib/WinJS/css/ui-light.min.css"));
                    loadCssPromiseList.push(Colors.loadCSSfile("css/opensans.css"));
                    loadCssPromiseList.push(loadCssPromise);
                    loadCssPromise = WinJS.Promise.join(loadCssPromiseList).then(function() {
                        ...
                        return Colors.loadCSSfile("css/default.css");
                    }).then(function() {
                        return Colors.loadCSSfile("css/index.css");
                    }).then(function() {
