...
(function () {
    ...
    WinJS.Namespace.define("AppData", {
        ...
        initXhrOptions: function(verb, url, isRegister) {
            var options = {
                type: verb,
                url: url,
                ...
                headers: {
                    "Authorization": "Basic " + btoa(AppData.getOnlineLogin(isRegister) + ":" + AppData.getOnlinePassword(isRegister))
                }
            };
            if (AppData._persistentStates.odata.corsQuirks) {
                options.headers["X-PINGOVER"] = "pingpong";
            } else {
                options.user = AppData.getOnlineLogin(isRegister);
                options.password = AppData.getOnlinePassword(isRegister);
            }
            return options;
        }
    });
    WinJS.Namespace.define("AppData", {
        ...
        /**
         * @function getIdFromLanguage
         * @param {number} language - A BCP-47 language tag.
         * @returns {string} A language identifier corresponding to the given BCP-47 language tag.
         * @memberof AppData
         * @description Use this function to convert a{@link https://tools.ietf.org/html/bcp47 BCP-47 language tag}, like "en-US", to a {@link https://msdn.microsoft.com/en-us/library/windows/desktop/dd318693.aspx language identifier}, like 0x409 (decimal 1033).
         *  If the given BCP-47 language tag is not supported, a default value of 0x409 (decimal 1033) will be returned.
         */
        getIdFromLanguage: function (languageTag) {
            ...
            for (var i = 0; i < languages.length; i++) {
                ...
                var language;
                if (pos >= 0) {
                    language = row.DOMCode.substr(0, pos).toUpperCase() + "-" + row.DOMCode.substr(pos + 1).toUpperCase();
                } else {
                    language = row.DOMCode.toUpperCase() + "-" + row.DOMCode.toUpperCase();
                }
        ...
        setConnectionProperties: function(db, properties, complete, error, bOnlyInsert) {
            Log.call(Log.l.trace, "setConnectionProperties.");
            ...
            var modifyProp = function(tx, prop, params) {
                ...
                Log.print(Log.l.info, "setConnectionProperties.modifyProp: " + [query, [prop]]);
                tx.executeSql(query, [prop], function(tx2, res2) {
                    var nextStmt = null;
                    Log.print(Log.l.info, "setConnectionProperties.modifyProp: result count=" + res2.rows.length);
                    ...
                    if (nextStmt) {
                        Log.print(Log.l.info, "setConnectionProperties.modifyProp: " + [nextStmt, params]);
                        tx2.executeSql(nextStmt, params, function (tx3, res3) {
                            Log.print(Log.l.info, "setConnectionProperties.modifyProp: " + "success!");
                        }, function (tx3, err) {
                            error(err);
                        });
                    }
            ...
            var ret = SQLite.xsql(db, stmt, values).then(function() {
                ...
                Log.print(Log.l.info, stmt);
                return SQLite.xsql(db, stmt, values).then(function () {
                    Log.print(Log.l.info, "setConnectionProperties: CREATE TABLE success!");
                }, function (err) {
                    Log.print(Log.l.error, "setConnectionProperties: CREATE TABLE: error!");
                    error(err);
                });
            }).then(function () {
                return SQLite.tx(db, function(tx) {
                    ...
                    Log.print(Log.l.trace, "setConnectionProperties.db.transaction complete!");
                    complete();
                }, function(err) {
                    Log.print(Log.l.error, "setConnectionProperties.db.transaction error!");
                    error(err);
                });
            });
            Log.ret(Log.l.trace);
