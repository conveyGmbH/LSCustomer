...
(function () {
            ...
            function PageControlNavigator(element, options) {
                    ...
                    if (element && element.style && splitViewContent) {
                        ...
                        if (navBarVisible) {
                            ...
                            if (NavigationBar.orientation === "horizontal") {
                                ...
                                if (!(AppBar.barElement && AppBar.barControl && AppBar.barControl.placement === "top" &&
                                      AppBar._commandList && AppBar._commandList.length > 0)) {
                                    top -= 2;
                                    height += 2;
                                }
                ...
                _navigating: function (args) {
                    ...
                    if (window.history) {
                        var state = {};
                        var title = "";
                        var url = window.location.href.split("?")[0] + "?page=" + Application.getPageId(this._nextPage);
    ...
    WinJS.Namespace.define("NavigationBar", {
        ...
        createViewPaneSubMenu: function (group, element, title) {
            ...
            if (subMenuFlyout &&
                subMenuList && subMenuList.winControl && element) {
                ...
                if (groups.length > 0) {
                    ...
                    WinJS.UI.Animation.slideRightIn(subMenuFlyout);
                }
            }
            Log.ret(Log.l.trace, "");
        },
                        ...
                        if (curSubMenu) {
                            ...
                            if (element) {
                                if (curSubMenu.disabled) {
                                    ...
                                    if (!WinJS.Utilities.hasClass(element, "win-disabled")) {
                                        WinJS.Utilities.addClass(element, "win-disabled");
                                    }
                                } else {
                                    if (WinJS.Utilities.hasClass(element, "win-nonselectable")) {
                                    ...
                                    if (WinJS.Utilities.hasClass(element, "win-disabled")) {
                                        WinJS.Utilities.removeClass(element, "win-disabled");
                                    }
                                }
                            }
                        }
        ...
        setMenuForGroups: function () {
            Log.call(Log.l.trace, "NavigationBar.");
            var i;
            var reloadSubMenu = false;
            var splitViewRoot = Application.navigator.splitViewRoot;
            var splitViewPane = Application.navigator.splitViewPane;
            var splitViewPaneScrollTop = splitViewPane && splitViewPane.scrollTop;
            var splitViewControl = splitViewRoot && splitViewRoot.winControl;
            ...
            if (navCommands) {
                ...
                if (NavigationBar.groups && NavigationBar.groups.length > 0) {
                    ...
                    WinJS.Promise.timeout(0).then(function() {
                        ...
                        if (reloadSubMenu) {
                            NavigationBar.handleSplitViewAfterClose();
                            return WinJS.Promise.timeout(50);
                        } else {
                            return WinJS.Promise.as();
                        }
                        ...
                        if (reloadSubMenu) {
                            ...
                            if (splitViewPane && splitViewPaneScrollTop > 0) {
