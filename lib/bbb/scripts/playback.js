...
*
*/
    function exitBBBPlayback() {
        window.BBBPlayback = {
            init: initBBBPlayback
        }
    }
    function initBBBPlayback() {
        var logger = window.Logger || console;

        var defaultCopyright = '<p>Recorded with <a target="_blank" href="https://bigbluebutton.org/">BigBlueButton</a>.</p><p>Use <a target="_blank" href="http://mozilla.org/firefox">Mozilla Firefox</a> or <a target="_blank" href="http://google.com/chrome/">Google Chrome</a> to play this recording.</p>';
     ...
     */
        ...
        var secondsToYouTubeFormat = function(secs) {
            var hours = Math.floor(secs / 3600);
            var minutes = Math.floor((secs - (hours * 3600)) / 60);
            var seconds = secs - (hours * 3600) - (minutes * 60);
     ...
     */
        ...
        function animateToCurrentSlide() {
            ...
            var thumbnailOffset = currentThumb.prop('offsetTop') -
     ...
     */
        function generateThumbnails() {
            ...
            var xmlList = shapesSVGContent.getElementsByTagName("image");
            var slideCount = 0;
            ...
            for (var k in imagesList) {
                if (Object.prototype.hasOwnProperty.call(imagesList, k)) {
                    $("#thumbnails").append(elementsMap[imagesList[k]]);
                }
            }
        };

        function loadCaptions() {
            BBBWriting.asyncRequest('GET', BBBWriting.captionsJSON)
                .then(function(response) {
                        ...
                        captions.forEach(function(caption) {
                            ...
                            track.setAttribute('src', BBBWriting.url + '/caption_' + caption['locale'] + '.vtt');
        ...
        function loadVideo() {
            ...
            webmsource.setAttribute('src', BBBWriting.url + '/video/webcams.webm');
            webmsource.setAttribute('type', 'video/webm; codecs="vp8.0, vorbis"');
            video.appendChild(webmsource);

            var mp4source = document.createElement("source");
            mp4source.setAttribute('src', BBBWriting.url + '/video/webcams.mp4');
            mp4source.setAttribute('type', 'video/mp4; codecs="avc1.42E01E"');
            video.appendChild(mp4source);

            video.setAttribute('data-timeline-sources', BBBWriting.chatXML);
        ...
        function loadAudio() {
            ...
            var webmsource = document.createElement("source");
            webmsource.setAttribute('src', BBBWriting.url + '/audio/audio.webm');
            ...
            oggsource.setAttribute('src', BBBWriting.url + '/audio/audio.ogg');
            ...
            audio.setAttribute('data-timeline-sources', BBBWriting.chatXML);
        ...
        function loadDeskshare() {
            ...
            var webmsource = document.createElement("source");
            webmsource.setAttribute('src', BBBWriting.url + '/deskshare/deskshare.webm');
            webmsource.setAttribute('type', 'video/webm; codecs="vp8.0, vorbis"');
            deskshareVideo.appendChild(webmsource);

            var mp4source = document.createElement("source");
            mp4source.setAttribute('src', BBBWriting.url + '/deskshare/deskshare.mp4');
            mp4source.setAttribute('type', 'video/mp4; codecs="avc1.42E01E"');
        ...
        function setMediaSync() {
            ...
            primaryMedia.on("pause",
                function() {
                    sync();
                });

            primaryMedia.on("seeking",
                function() {
                    ...
                    function() {
                        if (mediaSyncing || mediaSeeked) {
                            ...
                            for (var j = 0; j < allMedias.length; j++) {
                                allMediasAreReady &= (allMedias[j].media.readyState === 4);
                            }
    ...
    // Hack for mobile devices that not load media unless they are visible
        ...
        function playbackLoaded() {
            ...
            BBBWriting.loadMetadata();
            BBBWriting.checkMedias();
        ...
        function loadPlayback() {
            ...
            var isMobile = BBBWriting.mobileAndTabletCheck();
            if (isMobile) {
                logger.info("==Device is mobile");
                setTimeout(forceMediaEvents, BBBWriting.mobileTimeout);
            }

            BBBWriting.startLoadingBar();
            BBBWriting.loadData();
            loadBranding();
        ...
        function checkLoadedMedia() {
            ...
            if (isMediaReady(media)) {
                ...
                setTimeout(checkLoadedMedia, BBBWriting.mediaCheckInterval);
            }
        };

        function checkLoadedDeskshare() {
            var deskshare = $('#deskshare-video')[0];
            if (isMediaReady(deskshare)) {
                document.dispatchEvent(new CustomEvent('media-ready', { 'detail': 'deskshare' }));
            } else {
                setTimeout(checkLoadedDeskshare, BBBWriting.mediaCheckInterval);
            }
        };
        ...
        function loadBranding() {
            ...
            var metadata = BBBWriting.metadataXMLContent.getElementsByTagName("meta");

            if (metadata.length > 0) {
                metadata = metadata[0];

                var logoCandidates = metadata.getElementsByTagName("playback-logo-url");
                if (logoCandidates.length > 0) {
                    logo = logoCandidates[0].textContent;
                }

                var copyrightCandidates = metadata.getElementsByTagName("playback-copyright");
        ...
        function loadLogo(logo) {
            var logoURL = typeof logo !== 'undefined' ? logo : defaultLogo;
        ...
        function loadCopyright(copyright) {
            var copyrightText = typeof copyright !== 'undefined' ? copyright : defaultCopyright;
    ...
    // Swaps the positions of the presentation and the video
        ...
        function restoreCanvas() {
            ...
            var currentCanvas = BBBWriting.getCanvasFromImage(currentImageId);
            if (currentCanvas) {
        ...
        function restoreViewBoxSize() {
            ...
            var vboxVal = BBBWriting.getViewboxAtTime(t);
            if (vboxVal !== undefined) {
                BBBWriting.setViewBox(vboxVal);
            }
        };
        ...
        function restoreCursor(img) {
            ...
            BBBWriting.showCursor(true);
            var currentCursorVal = BBBWriting.getCursorAtTime(t);
            if (currentCursorVal) {
                BBBWriting.drawCursor(parseFloat(currentCursorVal[0]) / (imageWidth / 2), parseFloat(currentCursorVal[1]) / (imageHeight / 2));
            }
        };
    ...
    // need to fill 100% height.
        function resizeComponents() {
            logger.debug("==Resizing components");
            var mediaView = $("#recordedContent.mediaview");
            var availableHeight = mediaView.height();
            if (hasVideo) {
            ...
            if (mediaView.innerHeight > mediaView.innerWidth) {
                logger.debug("==Portrait mode");
                var mainSectionHeight = availableHeight * 0.6; // 60% for top bar
                $("#main-section").outerHeight(mainSectionHeight);
                var sideSectionHeight = availableHeight - mainSectionHeight;
                ...
                var chatHeight = availableHeight - $("#side-section").children("[data-swap]").outerHeight(true);
        ...
        document.addEventListener('playback-ready',
            function(event) {
        ...
        var bbbPlayback = {
            ...
            shapesSVGContent: {
                get: function() {
                    return shapesSVGContent;
                },
                set: function(value) {
                    shapesSVGContent = value;
                }
            },
            generateThumbnails: generateThumbnails,
            setMediaSync: setMediaSync,
            playbackLoaded: playbackLoaded,
            resizeComponents: resizeComponents,
            init: initBBBPlayback,
            exit: exitBBBPlayback
        }
        for (var prop in bbbPlayback) {
            if (bbbPlayback.hasOwnProperty(prop)) {
                var getter = bbbPlayback[prop].get;
                var setter = bbbPlayback[prop].set;
                if (typeof getter === "function" && typeof setter === "function") {
                    Object.defineProperty(bbbPlayback, prop, {     
                        get: getter,
                        set: setter
                    });
        ...
        window.BBBPlayback = bbbPlayback;
    }
