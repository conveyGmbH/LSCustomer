...
*
*/
    function exitBBBWriting() {
        window.BBBWriting = {
            init: initBBBWriting
        };
    }

    function initBBBWriting() {
        var logger = window.Logger || console;
        var params = getURLParameters();
        var meetingId = params.meetingId;
        var url = getFullURL();
        var metadataXML = url + '/metadata.xml';
        var shapesSVG = url + '/shapes.svg';
        var panzoomsXML = url + '/panzooms.xml';
        var cursorXML = url + '/cursor.xml';
        var deskshareXML = url + '/deskshare.xml';
        var textJSON = url + '/presentation_text.json';
        var captionsJSON = url + '/captions.json';
        var chatXML = url + '/slides_new.xml';
        var mediasURL = [
            '/video/webcams.webm',
            '/video/webcams.mp4',
        ...
        var mobileTimeout = 10 * 1000; // 10 seconds
        var mediaCheckInterval = 250;
    ...
    // Draw the cursor at a specific point
        ...
        function getShapesAtTime(time) {
            var shapesAtTime = timestampToId[time];
            var shapes = [];
            if (shapesAtTime != undefined) {
                for (var i = 0; i < shapesAtTime.length; i++) {
                    var id = shapesAtTime[i];
                    var shape = getSVGFile().getElementById(id);
        ...
        function getViewboxAtTime(time) {
            var currentTime = parseFloat(time);
            var showDeskshare = getDeskshareAtTime(time);
        ...
        function getDeskshareAtTime(time) {
            var show = false;
            if (BBBPlayback.hasDeskshare) {
                for (var m = 0; m < deskshareEvents.length; m++) {
                    var startTimestamp = deskshareEvents[m][0];
                    var stopTimestamp = deskshareEvents[m][1];
    ...
    // Find the key in the timestampToId object of the last entry at or before the provided timestamp
        ...
        function get_shapes_in_time(t, shapes) {
            ...
            var shapeInTime = timestampToId[timestampToIdIndex];
            if (shapeInTime != undefined) {
                for (var i = 0; i < shapeInTime.length; i++) {
                    var s = shapeInTime[i];
                    shapes[s.shape] = s.id;
                }
            }
        ...
        function runPopcorn() {
            ...
            popcorn.code({
                ...
                onFrame: function(options) {
                    ...
                    if ((!popcorn.paused() || popcorn.seeking()) && (Math.abs(currentTime - lastFrameTime) >= 0.1)) {
                        ...
                        var currentShapes = Object.create(mainShapeIds);
                        // And update it with current state of currently being drawn shapes
                        get_shapes_in_time(t, currentShapes);

                        // Update shape visibility status
                        for (var i = 0; i < shapesArray.length; i++) {
                            var aShape = shapesArray[i];
                            var time = parseFloat(aShape.getAttribute('timestamp'));
                            var shapeId = aShape.getAttribute('id');
                            var shapeI = aShape.getAttribute('shape');
                            var undo = parseFloat(aShape.getAttribute('undo'));

                            var shape = getSVGFile().getElementById(shapesArray[i].getAttribute("id"));

                            if (shape != null) {
                                if (
                                    // It's not the current version of the shape
                                    (shapeId !== currentShapes[shapeI]) ||
                                        // It's in the future
                                        (time > t) ||
                        ...
                        var nextImageId = getImageAtTime(t);
                        // Changing slide image
                        if (BBBPlayback.currentImageId && (BBBPlayback.currentImageId !== nextImageId) && nextImageId) {
                            logger.debug("==Changing image", nextImageId);
                            var img = getSVGFile().getElementById(BBBPlayback.currentImageId);
                            ...
                            if ($("#accEnabled").is(':checked')) {
                                ...
                                popcorn.listen(Popcorn.play, removeSlideChangeAttribute);
                            }
                            var currentCanvas = getCanvasFromImage(BBBPlayback.currentImageId);
                            if (currentCanvas) {
                                currentCanvas.setAttribute("display", "none");
                            }
                            var nextCanvas = getCanvasFromImage(nextImageId);
                            if (nextCanvas) {
                                nextCanvas.setAttribute("display", "");
                            }
                            BBBPlayback.currentImageId = nextImageId;
                        }

                        var image = getSVGFile().getElementById(BBBPlayback.currentImageId);
                        if (image) {
                            ...
                            var currentCursorVal = getCursorAtTime(t);
                            ...
                            BBBPlayback.currentImage = image;
                        }
    ...
    // Deskshare viewBox has the information to transform the canvas to place it above the video
        function adaptViewBoxToDeskshare(time) {
            var dimension = getDeskshareDimensionAtTime(time);
        ...
        function getDeskshareImage() {
            var images = getSVGFile().getElementsByTagName("image");
    ...
    // Transform canvas to fit the different deskshare video sizes
        function setTransform(time) {
            ...
            if (getDeskshareAtTime(time)) {
                ...
                var canvas = getCanvasFromImage(deskshareImage);
                if (canvas) {
        ...
        function processMetadataXML(response) {
            ...
            var metadata = metadataXMLContent.getElementsByTagName("meta");
        ...
        function processPanzoomsXML(response) {
            ...
            var panelements = response.responseXML.getElementsByTagName("recording");
            var panZoomArray = panelements[0].getElementsByTagName("event");
            var viewBoxes = response.responseXML.getElementsByTagName("viewBox");
        ...
        function processCursorXML(response) {
            ...
            var curelements = response.responseXML.getElementsByTagName("recording");
            var cursorArray = curelements[0].getElementsByTagName("event");
            var coords = response.responseXML.getElementsByTagName("cursor");
        ...
        function processShapesSVG(response) {
            ...
            var svgobj = document.createElement('div');
            ...
            BBBPlayback.shapesSVGContent = response.responseXML;

            // Getting all the event tags
            // Get an array of the elements for each "shape" in the drawing
            shapesArray = BBBPlayback.shapesSVGContent.querySelectorAll('g[class="shape"]');
            ...
            for (var j = 0; j < shapesArray.length; j++) {
                ...
                var shapeI = shape.getAttribute('shape');
                ...
                timestampToId[time].push({ id: id, shape: shapeI });

                mainShapeIds[shapeI] = id;
            }

            asyncRequest('GET', textJSON)
                .then(function(response) {
        ...
        function processDeskshareXML(response) {
            ...
            var deskshareElements = response.responseXML.getElementsByTagName("recording");
            var deskshareArray = deskshareElements[0].getElementsByTagName("event");
        ...
        function checkMediaURL(url) {
            if (!url) {
                logger.info("==media URL is undefined!");
                return;
            }
            logger.info("==media URL=", url);
            var xhr = new XMLHttpRequest();
        ...
        function initPopcorn() {
            ...
            firstLoad = false;
            BBBPlayback.generateThumbnails();
            ...
            popcorn.cue(2,
                    function() {
                        meetingDuration = parseFloat(popcorn.duration().toFixed(1));
        ...
        function processTextJSON(response) {
            ...
            var slidesText = JSON.parse(response.responseText);
            var shapeElements = BBBPlayback.shapesSVGContent.getElementsByTagName("svg");
            var images;
            if (typeof shapeElements[0].getElementsByClassName === "function") {
                images = shapeElements[0].getElementsByClassName("slide");
            } else {
                images = shapeElements[0].querySelectorAll(".slide");
            }
            for (var m = 0; m < images.length; m++) {
                var len = images[m].getAttribute("in").split(" ").length;
                for (var n = 0; n < len; n++) {
        ...
        function processAspectValue(vboxWidth, vboxHeight, time, lastAspectValue) {
            ...
            if (imageId !== undefined) {
                var image = getSVGFile().getElementById(imageId);
    ...
    // of its parent's area are hidden. If we var the svg occupy all presentation area
    ...
    // than we want to (i.e. more than was displayed in the conference).
        function resizeSlide() {
            ...
            if (BBBPlayback.currentImage) {
                ...
                logger.debug("==Size", { maxWidth: maxWidth, maxHeight: maxHeight });
            } else {
        ...
        function resizeDeskshare() {
            ...
            if ($deskshareVideo && deskshareVideo) {
                ...
                var maxWidth = aspectRatio * $deskshareVideo.parent().outerHeight();
                $deskshareVideo.css("max-width", maxWidth);

                var maxHeight = $deskshareVideo.parent().width() / aspectRatio;
        ...
        function getSVGFile() {
            var $slide = $("#slide");
            return slide.querySelectorAll('svg')[0];
        };
        ...
        document.addEventListener('media-ready',
            function(event) {
                logger.info("==Media ready", event.detail);
                if (BBBPlayback.mediaReady) return;
                switch (event.detail) {
                case 'video':
                    videoReady = true;
                    break;
        ...
        document.addEventListener('data-ready',
            function(event) {
                ...
                if (BBBPlayback.dataReady) return;
                switch (event.detail) {
                case 'svg':
                    svgReady = true;
                    break;
        ...
        var bbbWriting = {
            ...
            checkMedias: checkMedias,
            mobileAndTabletCheck: mobileAndTabletCheck,
            startLoadingBar: startLoadingBar,
            resizeSlide: resizeSlide,
            getViewboxAtTime: getViewboxAtTime,
            setViewBox: setViewBox,
            getCursorAtTime: getCursorAtTime,
            showCursor: showCursor,
            drawCursor: drawCursor,
            initPopcorn: initPopcorn,
            resizeContent: resizeContent,
            loadData: loadData,
            init: initBBBWriting,
            exit: exitBBBWriting
        }
        for (var prop in bbbWriting) {
            if (bbbWriting.hasOwnProperty(prop)) {
                var getter = bbbWriting[prop].get;
                var setter = bbbWriting[prop].set;
                if (typeof getter === "function" && typeof setter === "function") {
                    Object.defineProperty(bbbWriting, prop, {     
                        get: getter,
                        set: setter
                    });
